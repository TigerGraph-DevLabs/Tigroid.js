...
class Tigroid {
     ...
     *
     * @throws TigroidException if response contained a (server-side) error.
     */
    _errorCheck(res) {
...
*/
     ...
     *
     * @param {boolean} udts - If true, calls {@link _getUDTs}, i.e. includes User Defined Types in the schema details.
     ...
     * @param {string} attributes - Vertex attributes.
     *
     * @return A single number of accepted (successfully upserted) vertices (0 or 1).
     *
     * Data is upserted:
...
'}';
     ...
     */
    runInterpretedQuery(queryText, params = undefined) {
        queryText = queryText.replace("$graphname", this.graphname);
        if (this.debug) {
     ...
     */
    getToken(secret, setToken = true, lifetime= undefined) {
     ...
     *
     * Note: Saved but not installed queries will not be included. Installed queries that got invalidated due to schema or other changes are no longer considered to be in installed state.
     */
    getInstalledQueries() {
    ...
    getStatistics(seconds = 10, segments = 10) {
        ...
        return this._get({url: this.restppUrl + "/statistics/" + this.graphname + "?seconds=" + seconds.toString() + "&segment=" + segments.toString(), resKey: null});
    }
     ...
     * @param {number} restppPort - The port for REST++ queries.
     * @param {number} gsPort - The port of all other queries.
     * @param {string} apiToken - A token to use when making queries.
     * @param {boolean} debug - Enable debug output.
     */
    constructor({host = "http://localhost", graphname = "MyGraph", username = "tigergraph", password = "tigergraph", restppPort = 9000, gsPort = 14240, apiToken = undefined, debug = false}) {
        this.host       = host;
